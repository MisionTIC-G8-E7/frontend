import Footer from "../../components/Footer";
import Menu from "../../components/Menu";

const CanalesSoporte = () => {
  return (
    <>
      <Menu />
      <div className="row justify-content-center align-items-center p-5 fondo13 text-white">
        <div className="col-12 col-md-6 ">
          {/* <img
            className="img-fluid pt-4"
            src="https://abamobile.com/web/wp-content/uploads/2020/02/Desarrollo-y-creacion-de-apps-495x400.png"
            alt=""
          /> */}
        </div>
        <div className="col-12 col-md-1"></div>
        <div
          className="col-12 col-md-5"
          style={{ background: "rgb(0,0,0,0.5)" }}
        >
          <h1 className="text-center">TE OFRECEMOS CANALES DE SOPORTE</h1>
          <p>
            Lorem ipsum dolor sit amet, consectetur adipisicing elit. Numquam
            nam tempora modi, placeat ducimus rem, exercitationem pariatur vel
            temporibus illo magni asperiores consectetur distinctio itaque,
            optio eum deleniti omnis ullam! Facilis quidem non sed, id, rem
            beatae praesentium unde, commodi pariatur ullam sapiente iure! Odio
            omnis.
          </p>
        </div>
      </div>
      <div className="row justify-content-center align-items-center p-5 text-black h-auto">
        <div className="col-12 col-sm-1"></div>
        <div className="col-12 col-sm-10">
          <h2 className="fs-1 text-center">
            Estos son los ingredientes de un soporte técnico perfecto.
          </h2>
          <hr />
          <p>
            Lorem ipsum dolor sit amet consectetur adipisicing elit. Non
            reprehenderit adipisci vel aut voluptatem, quod accusantium!
            Laboriosam voluptate ipsum facere. Veniam tenetur maxime sequi
            perspiciatis asperiores aspernatur repellat voluptatibus nemo? Ipsam
            sed animi ab tenetur. Lorem, ipsum dolor sit amet consectetur
            adipisicing elit. Deleniti aperiam tenetur, aspernatur ipsum quos
            qui blanditiis in repudiandae doloribus unde ducimus, pariatur quas
            minima ut error placeat totam, debitis saepe. Lorem ipsum dolor sit
            amet consectetur, adipisicing elit. Voluptatibus fugiat ipsam ipsum
            a neque nisi officia autem repellat veritatis! Optio, possimus
            quisquam natus sit laboriosam velit eaque maxime sint dolorem.
          </p>
        </div>
        <div className="col-12 col-sm-1"></div>
      </div>

      <div className="row p-5 justify-content-center fondoCard2 align-items-center">
        <div className="col-md-12 col-lg-6">
          <img
            className="img-thumbnail"
            src="https://soportetecnicoparaempresas.com/wp-content/uploads/2019/07/Conectar-Resolver-al-instante.jpg"
            alt=""
          />
        </div>
        <div className="col-md-12 col-lg-6">
          <h2>Canales de soporte – Resolver al instante</h2>
          <p>
            Lorem ipsum dolor sit amet consectetur adipisicing elit.
            Voluptatibus qui itaque nisi, eos earum commodi incidunt debitis vel
            consequatur asperiores atque esse tenetur est quod quaerat aliquam?
            Enim, nemo iusto? Lorem ipsum dolor sit amet consectetur adipisicing
            elit. Et dolores mollitia sint deserunt reprehenderit
            exercitationem, provident voluptatum id quis facere? Unde similique
            recusandae sint, sed eveniet reprehenderit eligendi. Est,
            aspernatur? Lorem ipsum dolor sit amet, consectetur adipisicing
            elit. Sunt numquam mollitia molestiae quae deserunt pariatur
            possimus accusantium, illum delectus excepturi odio voluptas,
            adipisci deleniti laborum debitis? Nam saepe minima totam!
          </p>
        </div>

        <div className="col-md-12 col-lg-6">
          <h2>Asistencia técnica inteligente</h2>
          <p>
            Lorem ipsum dolor sit amet consectetur adipisicing elit.
            Voluptatibus qui itaque nisi, eos earum commodi incidunt debitis vel
            consequatur asperiores atque esse tenetur est quod quaerat aliquam?
            Enim, nemo iusto? Lorem ipsum dolor sit amet consectetur adipisicing
            elit. Et dolores mollitia sint deserunt reprehenderit
            exercitationem, provident voluptatum id quis facere? Unde similique
            recusandae sint, sed eveniet reprehenderit eligendi. Est,
            aspernatur? Lorem ipsum dolor sit amet, consectetur adipisicing
            elit. Sunt numquam mollitia molestiae quae deserunt pariatur
            possimus accusantium, illum delectus excepturi odio voluptas,
            adipisci deleniti laborum debitis? Nam saepe minima totam!
          </p>
        </div>
        <div className="col-md-12 col-lg-6">
          <img
            className="img-thumbnail"
            src="https://soportetecnicoparaempresas.com/wp-content/uploads/2019/07/Asistencia-t%C3%A9cnica-inteligente.jpg"
            alt=""
          />
        </div>

        <div className="col-md-12 col-lg-6">
          <img
            className="img-thumbnail"
            src="https://soportetecnicoparaempresas.com/wp-content/uploads/2019/07/Asesor%C3%ADa-personalizada.jpg"
            alt=""
          />
        </div>
        <div className="col-md-12 col-lg-6">
          <h2>Asesoría personalizada</h2>
          <p>
            Lorem ipsum dolor sit amet consectetur adipisicing elit.
            Voluptatibus qui itaque nisi, eos earum commodi incidunt debitis vel
            consequatur asperiores atque esse tenetur est quod quaerat aliquam?
            Enim, nemo iusto? Lorem ipsum dolor sit amet consectetur adipisicing
            elit. Et dolores mollitia sint deserunt reprehenderit
            exercitationem, provident voluptatum id quis facere? Unde similique
            recusandae sint, sed eveniet reprehenderit eligendi. Est,
            aspernatur? Lorem ipsum dolor sit amet, consectetur adipisicing
            elit. Sunt numquam mollitia molestiae quae deserunt pariatur
            possimus accusantium, illum delectus excepturi odio voluptas,
            adipisci deleniti laborum debitis? Nam saepe minima totam!
          </p>
        </div>
      </div>

      <div className="row p-5 justify-content-center align-items-center">
        <div className="col-12 col-md-1"></div>
        <div className="col-12 col-md-3">
          <img
            className="img-thumbnail rounded mx-auto d-block"
            src="https://soportetecnicoparaempresas.com/wp-content/uploads/2019/07/call-center-150x150.png"
            alt=""
          />
        </div>
        <div className="col-12 col-md-3 text-center fs-4">
          ¡Acompañamos a nuestros usuarios en todo momento!
        </div>
        <div className="col-12 col-md-1"></div>
        <div className="col-12 col-md-4">
          Lorem ipsum dolor sit, amet consectetur adipisicing elit. Dolorem nam
          deserunt, at, tenetur tempora eius cupiditate enim ea veritatis soluta
          incidunt temporibus voluptatibus recusandae repudiandae quo expedita
          sed earum dignissimos.
        </div>
      </div>

      <div className="row p-5 fondoCard2 justify-content-center align-items-center">
        <h2 className="fs-1 mb-5 text-center">Nuestros canales</h2>
        <div className="col-md-12 col-lg-4">
          <h3 className="text-center">TICKETS</h3>
          <img
            className="rounded mx-auto d-block"
            src="https://i.postimg.cc/FsdXTLtx/outline-content-paste-black-24dp.png"
            alt=""
          />
          <p>
            Lorem ipsum dolor sit amet consectetur, adipisicing elit. Excepturi
            iusto dicta exercitationem quibusdam facere eveniet corrupti itaque
            porro ratione maiores natus repudiandae ullam accusamus debitis,
            quis quos deserunt beatae autem.
          </p>
        </div>
        <div className="col-md-12 col-lg-4">
          <h3 className="text-center">CORREO ELECTRÓNICO</h3>
          <p>
            Lorem ipsum dolor sit, amet consectetur adipisicing elit. Dolore
            saepe, doloremque hic laborum facilis magnam possimus voluptatibus
            corrupti facere aspernatur obcaecati iusto, harum ullam, sit amet.
            Voluptates illum incidunt deleniti!
          </p>
        </div>
        <div className="col-md-12 col-lg-4">
          <h3 className="text-center">WHATSAPP</h3>
          <p>
            Lorem ipsum dolor sit amet consectetur adipisicing elit. Aliquam
            quia perferendis pariatur ipsa, totam reprehenderit possimus, vitae
            architecto consectetur delectus similique maiores fugit doloremque
            iusto nesciunt maxime commodi molestiae quo.
          </p>
        </div>

        <div className="col-md-12 col-lg-4">
          <h3 className="text-center">TELEFÓNICO</h3>
          <p>
            Lorem ipsum dolor sit amet consectetur, adipisicing elit. Excepturi
            iusto dicta exercitationem quibusdam facere eveniet corrupti itaque
            porro ratione maiores natus repudiandae ullam accusamus debitis,
            quis quos deserunt beatae autem.
          </p>
        </div>
        <div className="col-md-12 col-lg-4">
          <h3 className="text-center">SKYPE</h3>
          <p>
            Lorem ipsum dolor sit, amet consectetur adipisicing elit. Dolore
            saepe, doloremque hic laborum facilis magnam possimus voluptatibus
            corrupti facere aspernatur obcaecati iusto, harum ullam, sit amet.
            Voluptates illum incidunt deleniti!
          </p>
        </div>
        <div className="col-md-12 col-lg-4">
          <h3 className="text-center">ESCRITORIO REMOTO</h3>
          <p>
            Lorem ipsum dolor sit amet consectetur adipisicing elit. Aliquam
            quia perferendis pariatur ipsa, totam reprehenderit possimus, vitae
            architecto consectetur delectus similique maiores fugit doloremque
            iusto nesciunt maxime commodi molestiae quo.
          </p>
        </div>
      </div>

      <div className="row p-5 justify-content-center align-items-center">
        <h2 className="text-center">
          Tecnología para el desarrollo de una aplicación
        </h2>
        <div className="col-12">
          <img
            className="img-fluid rounded mx-auto d-block"
            src="https://abamobile.com/web/wp-content/uploads/2020/11/tecnologia-desarrollo-aplicaciones-moviles.png"
            alt=""
          />
        </div>
      </div>
      {/* <LlamadoAccion /> */}
      <Footer />
    </>
  );
};

export default CanalesSoporte;
